...
public class ProductRepositoryImpl implements Repository<Product>{
    ...
    public List<Product> listAllObj() throws SQLException {
        sql = "SELECT p.*, c.category_name FROM products_tbl p inner join category_tbl c" +
                " on (p.category_id = c.category_id) ORDER BY product_name";
    ...
    public Product byIdObj(Integer id) throws SQLException {
        sql = "SELECT p.*, c.category_name FROM products_tbl p inner join " +
                "category_tbl c on (p.category_id = c.category_id)WHERE p.category_id = ?";
